...
parser.add_argument("--show-na", action="store_true", help="Show variants even if there is no information for them in the databases")
...
def makeQuery(values, byID):
    if byID:
        if values[:2].lower() == "rs":
            query = 'q='+ values.lower() + '&scopes=dbsnp.rsid'
        elif values[:4].upper() == "ENSG":
            query = 'q=' + values.upper() + '&scopes=dbnsfp.ensembl.geneid'
        elif ">" in values:
            query = 'q=' + values
        else:
...
def saveReport(parsedJSON):
    ...
    if args.input == "":
        ...
        inputQuery = args.input
        
    html = html.replace("{input}", inputQuery)
    ...
    if results == 1:
        ...
        if byID:
            if 'notfound' in resultsJSON[0].keys():
                ...
                exit()
        parsedJSON, na_num = parseJSON(resultsJSON)
